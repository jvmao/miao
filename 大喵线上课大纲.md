...
* 设置搜索引擎为bing.com，google打不开，baidu搜索质量差
  ...
  * ==行内布局==
    ...
    * 行内布局的过程:
      ...
      * 然后将元素水平排列,让它们所有的基线对齐
        * 对齐的是匿名文本的基线
          * 如果没有匿名文本怎么办？
            * 总是想象出存在至少一个匿名文本，然后来对齐
        * 没有基线的元素（图片），m-box底部对齐基线
      * 然后在垂直方向上微调,基于vertical-align的值，这个属性只能给到**行内(块)元素**，给块元素没有效果，而是会对其内的行内元素生效（后续表单元格也可以给）
        * baseline 默认值
          * 该元素的基线与该行匿名文本的基线对齐
        * super
    ...
    * 一些需要说明的情况
      ...
      * 行内块元素的基线
        * 有文字时是最后一行文字的基线,但触发BFC后是布局框底部
        * 没有文字时是布局框(margin-box)的底部
        * 所以如果不注意就会造成布局看起来很乱的情况
...
* 第九章：颜色与背景
  ...
  * background
    ...
    * background-size
      ...
      * object-fit 调整图片的内容如何在img范围内展示的
        * img
        * video等
    ...
    * background-repeat
      * background-repeat
        ...
        * repeat-x/y
        * no-repeat
    * background-origin 图片起始平铺的盒子。以及size为百分比的的基准盒子
      * content-box
      * padding-box
      * border-box
      * 与box-sizing的关键字是对应的
    ...
    * css3 多背景
      * 分开写，合并写
    * background: <bg-repeat> <bg-img> <bg-origin>  <bg-size> / <bt-pos> , <bg-img> <bg-repeat> <bg-origin> <bg-clip>, <bg-img> <bg-repeat> <bg-origin> <bg-clip> bg-color;
    * 应用
      * 伪元素里的图片，
      * css sprite
      * x动画，菜单，小米网首页logo动画
...
* 表格布局
  ...
  * 抽象的列标签(col)只能控制4个样式,同时不能指定交互效果如hover
    * border
    * background
    * width
    * visibility
  ...
  * 对齐
    * vertical-align用在表单元格(td,th)上的效果
      * 调节是同一行表单元格的内容在垂直方向的对齐方式
      * top,bottom,middle,baseline


* 缓动
  * 元素的样式发生变化时不突变为新样式,而是缓慢变到目标样式
  * 由transition系列属性控制
  * transition-duration
    * 设置缓动持续的时间
  * transition-property
...
* 伪元素
  * 每个元素内都有一前一后两个隐藏的子元素(自闭合标签没有)
  * 先用选择器选到相应元素，再在后面加上::before,::after即可分别选中它的两个伪元素
  * 如span::before, span::after将选中所有span的前后伪元素
    * 选择器严格来说应该是双冒号,但由于历史遗留原因,单冒号也是可以的,但实际上单冒号是**伪类**的选择器
  * 伪类与伪元素的区别
    * 伪类是实际元素的隐含状态或交互状态
    * 伪元素是每个元素前后多出来的隐藏元素
  * 伪元素选择器的后面不能再出现其它选择器了:
    * 因为伪元素里面不会再有其它元素了
    * span::before + p {} 这是无意义的,
    * 伪元素不能被:hover匹配
      * span::before:hover 是无效的
      * span:hover::before 但这种写法是有效的,表示span被hover时它的伪元素的样式
    * 伪元素选择器本身不计优先级,伪元素样式冲突时看其前序选择器的优先级（选到父元素的那个选择器的会优先级）
  * 不设置内容时它们不会出现
    ...
    * 伪元素只在css中存在，在js中不存在（无法用js与伪元素产生交互或关联），内部的文字也无法选中
  * 内容可以设置为文本，图片但一般只设置为文字
  * 什么时候用
    * 一系列类似元素统一装饰性样式(往往不需要交互或直接跟父元素交互)，但该样式需要添加额外元素时，即可用伪元素（有时不用伪元素而是只设置css也能达到效果，则可以不用伪元素）
      * 一般只有装饰性效果使用,可交互效果（如点击弹出内容等）不使用,因为伪元素不可交互,文字也不可选中
...
* 什么时候后面的元素能影响前面的元素？
  * 一般来说后面的元素不能决定前面元素的样子
  * 里面的元素也不能最终影响到外面元素的样子

* css变量，又叫css自定义属性
  * css变量必须以--开头
  * css变量会自动继承给后代们
    * 后代如果有同名的变量则当然直接用自己的了
  * 使用变量时要用var函数包起来才可以，不能直接使用的
...
* 浮动布局
  ...
  * 浮动元素同时处于常规流内与常规流外
    * 即有些元素能感知到它而有些元素感知不到它
      ...
      * **可以通过一些手段让它能感知到**
        * 什么手段?
  * 浮动元素无需设置display:inline-block等，元素浮动时外部布局变为浮动模式，内部保留原来的布局方式
    * 此时display只改变元素元素的内部布局
    * 外部布局由浮动来决定
  * 浮动这个词如何理解？
    * 浮动元素只能处于能够摆放文字（即行内内容）的上下文中
  * 浮动与定位一起使用
    * 可以与相对定位一起用
      * 相对定位是相对于浮动之后的位置去偏移
        * 于相对于自己浮动之后的位置去偏移
  ...
  * 清除浮动
    ...
    * 只能用在块框上（浮动元素也可以用）（就是行内元素不能用）
  * 闭合浮动(float closure)（非官方/书本概念）
    * 元素变大(往往是变高)以包裹其内的所有浮动元素以避免影响其它元素
    * 一般通过触发bfc实现
      * overflow: hidden;
      * display: flow-root;
      * float: left; 浮动元素也会触发bfc 一般不使用这种办法，因为它会让该自身也浮动，又带了其包含需要闭合浮动的问题
    * 也可以借助清除浮动实现
      * 在需要闭合浮动的元素最后加一个块元素并且清除该块元素的浮动，该块元素下移就会撑大需要闭合浮动的元素（这个块元素最终也可以用after伪元素来实现）
  * 浮动的一般应用
    * 横向布局（类似行内块）
...
* flexbox 弹性盒模型
  ...
  * 一些需要讨论的问题
    ...
    * 横向无法撑开包含块(后续的grid算是解决了这个问题)（2025,这个问题也不存在了）
    * 弹性盒模型中的overflow
  * 案例
    * 小米产品列表
    * 主页大图旁边的产品子菜单
...
* 媒体查询,我们可以根据显示在不同设备上(屏幕还是打印)/窗口的宽度/高度(的范围)/比例/(手机)屏幕方向/像素比(一个css像素等于多少物理像素/cm/英寸等）例等来启用或禁用一段css代码
  * **根据屏幕的宽度来让页面的布局发生变化**
...
* web字体
  ...
  * css3的这个特性可以让你加载线上字体
    ...
    @font-face {
      ...
      font-family: TheName;
      font-weight: normal;// 告诉浏览器这个字体文件的属性
      font-style: italic; // 告诉浏览器这个字体文件的属性
    }
    @font-face {
      写相对路径的时候是相对这句代码所在的文件的文件夹
      src: url(path/to/the/font/file.ttf/eot/woff/otf);
  ...
  * 与css sprite相比有什么好处
    * 矢量图,放大也清晰
    * 文件更小(因为是矢量图,不是存每个点的颜色,而是存组成图形的线条的贝塞尔曲线的点)(矢量图在表达几何形状时体积是远小于像素图的)(如果非要拿矢量图表达一张照片,体积会比像素图要大)
    * 维护时不用担心冲突
    * 可以变颜色(因为是文字)
  * 缺点:
...
* 2d&3d
  ...
  * 支持的变换操作：
    ...
    * skewx/y() 倾斜
    * scale(),scalex/y() 缩放
    * translate(),translatex/y() 平移，百分比是相对于元素自己的border-box
    * matrix() 矩阵变换
  ...
  * 3d变换
    ...
    * 支持的变换：
      * rotatex/y/z(degree)
      * rotate3d(x,y,z,degree)沿着一个向量旋转
      * translatex/y/z()
      * scale
      * 没有skew
    ...
    * transform-style: preserve-3d/none; 变换时是否保留3d效果还是只显示3d的投影效果
    * transform-origin: x y z;设置坐标原点相对元素的位置
    * perspective  观察距离
      * persepective属性与perspective()变换的区别
    * perspective-origin 眼睛对xoy平面做垂线的垂点。
...
* 动画与缓动
  ...
  * animation-fill-mode: none,backwards,forwards,both;开始前和结束后的状态
...
* 移动端布局(即在手机上显示的网页)
  * 手机浏览器一般是全屏显示并且宽度不可调节
    * 手机上的所有app一般来说都是全屏显示的
    * 在电脑上一般软件都是可以调节尺寸的
  * 那手机上浏览器的宽度为多少呢?
    * 并不是手机实际物理分辨率的值
  * 可以自己测试
    * 如何测?用一张图片
  * 不同手机宽度不一样,最大到最小差别还蛮大
    * 一般来讲手机越大浏览器的宽度越宽
  * 可以做流式布局
  * 但不一定满足要求
...
移动端布局：
  ...
  移动端布局跟pc端/桌面端布局有着很大的不同
  pc上屏幕都足够大
  但手机尺寸及分辨率都各不相同且物理尺寸很小
  于是就造成了在pc和手机上完全不同的布局策略
  在pc上一般都是定宽居中
    而且这个定宽是比一般电脑的显示器要小的
    主体区域不管在哪个电脑上都是一样的大小
      只是留白不一样
  ...
  手机上的布局都不是这样的
    ...
    手机上也没有窗口的概念的
      一般app都是占满全屏的
    手机屏幕物理尺寸各不相同，分辨率也各不相同
    手机浏览器一般是将整个屏幕做为视口的
      当手机屏幕就是整个浏览器窗口的时候
        它的css像素宽度是多少？
          其实不同电脑在浏览器全屏的时候，视口的css尺寸也是不一样的
            但是在电脑上，视口大小都是足够大的
              所以我们才能定宽设计，因为设计的宽度往往是小于屏幕宽度的
        它的css像素宽度是多少？
          由于手机物理像素的大小都非常小，所以手机网页代码中的1px往往不是手机屏幕的一个物理像素
          为了编码习惯，一般厂商是将手机浏览器的视口宽度设置为相同物理尺寸下电脑显示器的像素数量：
            手机屏幕一般是6cm宽左右，而普通的电脑显示器6cm宽一般是300像素左右
            再考虑手机离人眼更近，所以一般将视口大小设置成350css像素左右
              但也是有浮动的，跟厂商以及手机的物理尺寸有关，一般在300到500之间
              即，不同型号的手机视口宽度不一样。
              但是，我们又希望我们的网页，能占满宽度

viewport声明标签
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  当页面没有声明这个标签时，因为历史遗留原因，手机浏览器会认为这是为电脑设计的网页
    所以会用电脑上典型的浏览器窗口来渲染页面，一般在1000px左右
  当页面声明这个标签时，手机浏览器会认为这个页面是为移动端设计的，所以会按声明来
    width=device-width 就是用来设定手机浏览器窗口宽度的，可以为数字，也可以为一个预置值：device-width
      device-width为出厂设定的，不同手机不一样，一般屏幕的物理尺寸越大，它越大
    当它为数字时，也就是说，开发者可以通过这个标签将所有手机浏览器的视口宽度设置为相同的
  但是，比较旧的手机（2017年之前的，安卓5.0以下）不能将该值设置为数字，而只能写device-width
    当不能写数字时，相当于不同手机宽度又不一样了！！！

典型的手机网页的设计：
  另一种是，像pc上的流式布局一样，按钮大小相对固定，但屏幕越大，一行能显示的按钮也越多，字也越多
    类似github
      直接写content=device-width，然后使用常规块元素布局，px为单位开发即可
  一种是类型手机软件一样，在不同的手机上，按钮大小不一样，但整体布局比例一样
    类似小米移动端首页
      如果设备支持，那么设计图有多宽，content=多宽，这样一来，相当于开发者面对的所有手机都有相同的浏览器窗口
        直接用量出来的尺寸加px单位就可以了。
      如果设备不支持content写为数值，则可以使用vw开发，先将设计图缩放为宽度1000
        然后量出设计图中354像素时，就在代码中写35.4vw
        以此类推，所有长度单位都使用vw，包括字号，边框，宽高。

        但此方案的问题是，对设计图进行了缩放，图片内的一些边缘会发糊，导致测量尺寸不准
        我们希望，即不对设计进行缩放，也能够直接将测量出来的尺寸直接用在代码里
          假设设计稿宽度为X，那么我们希望
            Xrem = 屏幕宽度 = 100vw
            即 html {
              font-size: calc(100vw / X);
            }
          又由于100vw / X后的结果可能很小，小于1px，有些浏览器会限制最小字号
          所以
            html {
              font-size: calc(100vw / X * 100);
            }
          这样一来，我们不用缩放设计图，而是直接使用从设计图里测量出来的尺寸，加上rem单位即可
          在不支持vw或calc的浏览器上，通过js读到窗口宽度的px单位，除以X*100，设置到html的字号上就行了

  少见的第三种设计：
    屏幕越大字显示的越多（类似github）
    但屏幕越大，按钮也越大（类似小米）
    即除了文字以外，其它都是缩放的
    那就除主体文字以外，都使用rem单位，但主体文字使用px，一般16px
...
* 混合模式
  ...
  * mix-blend-mode
    * 元素自己跟它底下的元素颜色如何进行混合
* 滤镜
  * filter: ;与transform差不多,区别是它只做颜色上的变化,也不影响布局,也是从前往后变,后面的变化基于前续变化的结果
    * filter: 模糊  增加对比度  减少饱和度  变旧;
    * drop-shadow()滤镜:对元素不透明投射阴影,半透明的地方也有影阴,但阴影更淡,完全不透明的地方阴影最重
* 蒙版
...
* -webkit-前缀x
  ```
    -webkit-background-clip: ; blink
    -moz-background-clip: ;    gekco
    -ms-background-clip: ;
    -o-background-clip: ;
    各自理解的效果加上各自的前缀
    等到大家理解都相同的时候，再用完全没有前缀的写法
    但是浏览器们发现一旦浏览器支持前缀写法，就会有人开始用
    而且用了就再不删了
    但是带前缀的写法本来只是想让你试用一段时间后就删除的，使用新的没有前缀的属性替代
    但是由于网站不再删除它，所以浏览器也不能删除对前缀属性的支持
    就造成了现在的结果
    为了避免更多这这样的问题，浏览器现在不会新增任何带前缀的属性了
    那新的属性如何试用呢？
    答：在浏览器里隐藏界面专门开启试用选项。选项开启后试用也不需要加前缀，但其它用户用不了
    由于存在不少-webkit-前缀的属性
    后来火狐浏览器也可以识别遗留的-webkit-前缀的属性
  ```
...
# JavaScript
  * 关于书本《Eloquent JavaScript》
    * Marijn Haverbeke
    * Code Mirror的作者
      * CodeMirror被内置进谷歌浏览器了
    * 为什么用书而不是像某些机构一样自己准备所谓的ppt？
      * 实际上我css部分也是用的书本
      * 原因再简单不过，我选的这几本书，作者都比我强很多倍
        * 他们写的书自然是足够好的，并且深入浅出
        * 就算对于新手不友好，至少在有我讲的情况下，是绝佳的学习资料
        * 绝对不用担心内容的顺序问题
        * 绝对不用担心重要内容的缺失
        * 绝对不用担心内容的系统性和全面性
      * 拿这本书来说，书中几乎没有废话
      * 从小学到大学都有书本，怎么到了IT培训机构，没书本了？
  * JS是【编程语言】，不同于HTML（标记语言）和CSS（样式语言）
    * 不会因为你会了html和css而相当于有半点js基础
    * 它们是完全不同的，实际上正常的学习顺序应该是先学编程语言，再学html/css
    * 有些人认为js难（相对于html/css）是因为他们把它当成类似html与css的东西了
    * 与html/css一样的唯一地方就是想学会就得learn by doing
  * 现代社会编程语言无处不在
    * 所有自动控制的设备都是程序在驱动
      * 汽车
      * 蓝牙耳机
      ...
      * 空调
      * 房卡
      * 可以说很难找到一个通了电但里面没有程序的设备了
  * 程序最早设计是为了处理大量数运算符据
  * 要处理数据，首先面要把数据表达在程序语言里
  * 数据有不同的种类（温度，长度），且之间可以进行运算
  * 所以第一章的内容是“值，类型，与”
